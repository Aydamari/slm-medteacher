...
class LeadIdentifier:
        ...
        if num_rhythm_leads > 0:
            ...
            try:
                ...
                for i_r, i_c in zip(row_idx, col_idx):
                    ...
                    logger.info(f"Rhythm {i_r} → Canonical {canonical_order[i_c]} (corr={corr_val:.2f})")
                    canonical[i_c, :] = lines[-num_rhythm_leads + i_r, :]
            except ValueError:
                if self.debug:
                    logger.warning("Linear sum assignment failed, possibly due to NaN values in rhythm correlations.")
    ...
    def __call__(
        ...
        if len(detected) <= 2:
            ...
            match = self._match_layout(detected, rows_in_layout, layouts, check_flipped)

        if "layout" not in match:
            logger.warning(f"No matching layout found, defaulting to first layout: {list(layouts.keys())[0]}")
