...
class SignalExtractor:
    ...
    def __init__(
        ...
        label_thresh: float = 0.05, # Reduzido de 0.1 para capturar traços finos em PC
        ...
        apply_smoothing: bool = False, # Novo parâmetro para controlar o blur
    ) -> None:
    ...
    def __call__(self, feature_map: torch.Tensor) -> torch.Tensor:
        ...
        if self.num_peaks != len(merged_lines):
            logger.warning(
                f"Number of peaks ({self.num_peaks}) does not match number of merged lines ({len(merged_lines)})."
            )
        return merged_lines
    ...
    def _autodetect_num_peaks(self, fmap: torch.Tensor) -> int:
        ...
        if most_common_count < line_counts.shape[0] * 0.5:
            logger.warning(
                f"Autodetected number of peaks ({most_common_value}) is not reliable, "
